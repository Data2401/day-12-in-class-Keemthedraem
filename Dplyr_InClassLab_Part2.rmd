...
```{r}
my_flights %>% filter(origin == "JFK", dest == "SEA") %>% summarise( avg_air = mean(air_time, na.rm = TRUE), min_air = min(air_time, na.rm = TRUE),  max_air = max(air_time, na.rm = TRUE)
  )
...
```{r}
my_flights %>% filter(dest %in% c("HOU", "IAH")) %>% group_by(origin) %>% summarise(mean_arr_delay = mean(arr_delay, na.rm = TRUE))
...
```{r}
my_flights %>% group_by(origin) %>%  slice_max(dep_delay, n = 1, with_ties = FALSE)
...
```{r}
starwars %>% group_by(eye_color) %>% summarise(avg_height = mean(height, na.rm = TRUE))
...
```{r}
starwars %>% filter(eye_color == "brown") %>% select(name, height)
